...
SetMaterial(game_state* GameState, const camera* Camera, const material* Material)
{
  ...
  else if(Material->Common.ShaderType == SHADER_Toon)
  {
    GLuint ToonShaderID = GameState->Resources.GetShader(GameState->R.ShaderToon);
    glUseProgram(ToonShaderID);

    glUniform1i(glGetUniformLocation(ToonShaderID, "flags"), Material->Toon.Flags);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "material.ambientColor"), 1,
                 (float*)&Material->Toon.AmbientColor);
    glUniform4fv(glGetUniformLocation(ToonShaderID, "material.diffuseColor"), 1,
                 (float*)&Material->Toon.DiffuseColor);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "material.specularColor"), 1,
                 (float*)&Material->Toon.SpecularColor);
    glUniform1i(glGetUniformLocation(ToonShaderID, "material.diffuseMap"), 0);
    glUniform1i(glGetUniformLocation(ToonShaderID, "material.specularMap"), 1);
    glUniform1i(glGetUniformLocation(ToonShaderID, "material.normalMap"), 2);
    glUniform1i(glGetUniformLocation(ToonShaderID, "shadowMap"), 3);
    glUniform1f(glGetUniformLocation(ToonShaderID, "material.shininess"),
                Material->Toon.Shininess);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "lightPosition"), 1,
                 (float*)&GameState->R.LightPosition);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "light.ambient"), 1,
                 (float*)&GameState->R.LightAmbientColor);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "light.diffuse"), 1,
                 (float*)&GameState->R.LightDiffuseColor);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "light.specular"), 1,
                 (float*)&GameState->R.LightSpecularColor);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "sunPosition"), 1,
                 (float*)&GameState->R.SunPosition);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "sun.ambient"), 1,
                 (float*)&GameState->R.SunAmbientColor);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "sun.diffuse"), 1,
                 (float*)&GameState->R.SunDiffuseColor);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "sun.specular"), 1,
                 (float*)&GameState->R.SunSpecularColor);
    glUniform3fv(glGetUniformLocation(ToonShaderID, "cameraPosition"), 1,
                 (float*)&Camera->Position);
    glUniformMatrix4fv(glGetUniformLocation(ToonShaderID, "mat_sun_vp"), 1, GL_FALSE,
                       GameState->R.SunVPMatrix.e);
    glUniform1i(glGetUniformLocation(ToonShaderID, "levelCount"), Material->Toon.LevelCount);

    assert(
      ((Material->Toon.Flags & TOON_UseDiffuseMap) && Material->Toon.DiffuseMapID.Value > 0) ||
      !(Material->Toon.Flags & TOON_UseDiffuseMap));
    assert(
      ((Material->Toon.Flags & TOON_UseSpecularMap) && Material->Toon.SpecularMapID.Value > 0) ||
      !(Material->Toon.Flags & TOON_UseSpecularMap));
    assert(
      ((Material->Toon.Flags & TOON_UseNormalMap) && Material->Toon.NormalMapID.Value > 0) ||
      !(Material->Toon.Flags & TOON_UseNormalMap));

    uint32_t DiffuseTexture = (Material->Toon.Flags & TOON_UseDiffuseMap)
                                ? GameState->Resources.GetTexture(Material->Toon.DiffuseMapID)
                                : 0;
    uint32_t SpecularTexture = (Material->Toon.Flags & TOON_UseSpecularMap)
                                 ? GameState->Resources.GetTexture(Material->Toon.SpecularMapID)
                                 : 0;
    uint32_t NormalTexture = (Material->Toon.Flags & TOON_UseNormalMap)
                               ? GameState->Resources.GetTexture(Material->Toon.NormalMapID)
                               : 0;
    uint32_t ShadowTexture = GameState->R.DepthMapTexture;
