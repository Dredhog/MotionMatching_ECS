...
void
ExportMaterials(material* Materials, int32_t MaterialCount)
{
  for(int i = 0; i < MaterialCount; i++)
  {
    char FileName[30];
    sprintf(FileName, "material%d", i);
    ...
    if(Materials[i].Common.ShaderType == SHADER_Phong)
    {
      ...
      {
        fprintf(FilePointer, "\tmap_Ka %s\n", "Path");
        fprintf(FilePointer, "\tmap_Kd %s\n", "Path");
      }
      else if(Materials[i].Phong.Flags & PHONG_UseSpecularMap)
      {
        fprintf(FilePointer, "\tmap_Ks %s\n", "Path");
      }
      else if(Materials[i].Phong.Flags & PHONG_UseNormalMap)
      {
