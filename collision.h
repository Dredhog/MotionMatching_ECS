...
vec3
Support(vec3* ShapeA, int32_t LengthA, vec3* ShapeB, int32_t LengthB, vec3 Direction)
{
  float   MaxA   = Math::Dot(ShapeA[0], Direction);
  float   MinB   = Math::Dot(ShapeB[0], Direction);
  int32_t IndexA = 0;
  int32_t IndexB = 0;
  ...
  {
    DotProduct = Math::Dot(ShapeA[i], Direction);
    if(DotProduct > MaxA)
    {
      MaxA   = DotProduct;
      IndexA = i;
  ...
  {
    DotProduct = Math::Dot(ShapeB[i], Direction);
    if(DotProduct < MinB)
    {
      MinB   = DotProduct;
      IndexB = i;
  ...
  return ShapeA[IndexA] - ShapeB[IndexB];
}
...
bool
GJK(vec3* Simplex, int32_t* SimplexOrder, vec3* ShapeA, int32_t LengthA, vec3* ShapeB, int32_t LengthB)
{
  Simplex[0]     = ShapeA[0] - ShapeB[0];
  vec3 Direction = -Simplex[0];
  *SimplexOrder  = 0;

  for(;;)
  {
    vec3 A = Support(ShapeA, LengthA, ShapeB, LengthB, Direction);
...
FindEdge(edge* Edges, int32_t EdgeCount, edge TargetEdge)
{
  ...
  {
    // TODO(rytis): Figure out why two non-reverse edge scenario is possible (likely fault: wrong simplex vertex order)
...
vec3
EPA(vec3* Simplex, int32_t SimplexLength, vec3* ShapeA, int32_t LengthA, vec3* ShapeB, int32_t LengthB)
{
  vec3 Result;
  ...
  for(;;)
  {
    ...
    vec3 NewPoint = Support(ShapeA, LengthA, ShapeB, LengthB, Polytope[TriangleIndex].Normal);
