...
void
PerformPostProcessing(game_state* GameState)
{
  ...
  if(GameState->R.PPEffects & (POST_HDRTonemap | POST_Bloom) ||
    ...
    if(GameState->R.PPEffects & POST_Bloom)
    {
      ...
      for(int i = 0; i < GameState->R.BloomBlurIterationCount; i++)
      {
        // Horizontal blur bright regions
        {
          ...
          GLuint ShaderPostBloomBlurID = GameState->Resources.GetShader(GameState->R.PostBloomBlur);
      ...
      {
        {
          ...
          glUniform1i(glGetUniformLocation(ShaderBloomTonemapID, "u_ScatteredLightMap"), tex_index);
        }
      }
  ...
  if(GameState->R.PPEffects)
  {
    if(GameState->R.PPEffects & POST_EdgeOutline)
    {
      ...
      glUniform1f(glGetUniformLocation(PostEdgeOutlineShaderID, "cameraFarClipPlane"),
    ...
    if(GameState->R.PPEffects & (POST_Blur | POST_DepthOfField))
    {
      ...
      if(GameState->R.PPEffects & POST_DepthOfField)
      {
        ...
        GLuint ShaderDepthOfFieldID = GameState->Resources.GetShader(GameState->R.PostDepthOfField);
    ...
    {
      GLuint PostNoiseShaderID = GameState->Resources.GetShader(GameState->R.PostNoise);
    ...
    if(GameState->R.PPEffects & POST_Test)
    {
      ...
      GLuint PostTestShaderID = GameState->Resources.GetShader(GameState->R.PostShaderTest);
