...
namespace Mesh
{
  const int MAX_FLOAT_COUNT = 100000000;
  ...
  void PrintMesh(const mesh* Mesh)
  {
    ...
    printf("PosOffset : %d, UVOffset : %d, NormOffset: %d\n", Mesh->Offsets[0], Mesh->Offsets[1], Mesh->Offsets[2]);
    for(int i = 0; i < Mesh->VerticeCount; i++)
    {
      int VertexStartFloat = i * Mesh->FloatsPerVertex;

      printf("%d: P:{ %5.2f %5.2f %5.2f }\t", i, (double)Mesh->Floats[VertexStartFloat + Mesh->Offsets[0]],
             (double)Mesh->Floats[VertexStartFloat + Mesh->Offsets[0] + 1], (double)Mesh->Floats[VertexStartFloat + Mesh->Offsets[0] + 2]);
      if(Mesh->UseUVs)
      {
      ...
      if(Mesh->UseNormals)
      {
        printf("N:{ %5.2f %5.2f %5.2f }\n", (double)Mesh->Floats[VertexStartFloat + Mesh->Offsets[2]],
               (double)Mesh->Floats[VertexStartFloat + Mesh->Offsets[2] + 1], (double)Mesh->Floats[VertexStartFloat + Mesh->Offsets[2] + 2]);
      }
    }
    printf("INDICES\n");
  ...
  mesh ReadOBJMesh(const char* FileName, const bool UseUVs, const bool UseNormals)
  {
    ...
    while(!feof(FileHandle))
    {
      ...
      if(FirstChar == 'v')
      {
        ...
        if(SecondChar == ' ')
        {
          sscanf(&ReadLine[2], "%f %f %f ", &Positions[3 * PositionCount], &Positions[3 * PositionCount + 1], &Positions[3 * PositionCount + 2]);
        ...
        {
          sscanf(&ReadLine[2], " %f %f %f ", &Normals[3 * NormalCount], &Normals[3 * NormalCount + 1], &Normals[3 * NormalCount + 2]);
          ++NormalCount;
        }
      }
      else if(FirstChar == 'f')
      {
        ...
        if(UseUVs && UseNormals)
        {
          sscanf(&ReadLine[2], " %d/%d/%d %d/%d/%d %d/%d/%d ", &V1[0], &V1[1], &V1[2], &V2[0], &V2[1], &V2[2], &V3[0], &V3[1], &V3[2]);
        }
        else if(!UseUVs && UseNormals)
        {
          sscanf(&ReadLine[2], " %d//%d %d//%d %d//%d ", &V1[0], &V1[2], &V2[0], &V2[2], &V3[0], &V3[2]);
        }
        else if(UseUVs && !UseNormals)
        {
          sscanf(&ReadLine[2], " %d/%d/ %d/%d/ %d/%d/ ", &V1[0], &V1[1], &V2[0], &V2[1], &V3[0], &V3[1]);
        }
        else
        {
    ...
    float* MeshMemory = (float*)malloc(Mesh.VerticeCount * Mesh.FloatsPerVertex * sizeof(float) + Mesh.IndiceCount * sizeof(uint32_t));
    if(!MeshMemory)
    {
      printf("Read OBJ error: failed to allocate %lu bytes for final vertice memory\n",
             Mesh.VerticeCount * Mesh.FloatsPerVertex * sizeof(float) + Mesh.IndiceCount * sizeof(uint32_t));
      fclose(FileHandle);
      free(Floats);
      return {};
    }
...
#if 0
      printf("Vertex: %d\n", i);
      printf("ReadingFrom: %d, Writing to: %d\n", (3 * (Indices[IndexTripletIndex] - 1) + 0), (i * Mesh.FloatsPerVertex + Mesh.Offsets[0] + 0));
      printf("ReadingFrom: %d, Writing to: %d\n", (3 * (Indices[IndexTripletIndex] - 1) + 1), (i * Mesh.FloatsPerVertex + Mesh.Offsets[0] + 1));
      printf("ReadingFrom: %d, Writing to: %d\n", (3 * (Indices[IndexTripletIndex] - 1) + 2), (i * Mesh.FloatsPerVertex + Mesh.Offsets[0] + 2));
#endif

      Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[0] + 0] = Positions[3 * (Indices[IndexTripletIndex] - 1) + 0];
      Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[0] + 1] = Positions[3 * (Indices[IndexTripletIndex] - 1) + 1];
      Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[0] + 2] = Positions[3 * (Indices[IndexTripletIndex] - 1) + 2];
      if(UseUVs)
      {
        Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[1] + 0] = UVs[2 * (Indices[IndexTripletIndex + 1] - 1) + 0];
        Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[1] + 1] = UVs[2 * (Indices[IndexTripletIndex + 1] - 1) + 1];
      }
      if(UseNormals)
      {
        Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[2] + 0] = Normals[3 * (Indices[IndexTripletIndex + 2] - 1) + 0];
        Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[2] + 1] = Normals[3 * (Indices[IndexTripletIndex + 2] - 1) + 1];
        Mesh.Floats[i * Mesh.FloatsPerVertex + Mesh.Offsets[2] + 2] = Normals[3 * (Indices[IndexTripletIndex + 2] - 1) + 2];
      }
    }
